...
public class SLM_QTEEvents : MonoBehaviour
{
         ...
         if (timer < presets[curid].time || presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.reaction)
         {
            ...
            if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.basic)
            {
               if (stage < presets[curid].buttonIds.Count)
               {
                  ...
                  for (int i = 0; i < buttons.Count; i++)
                  {
                     if (!butts.Contains(i.ToString()) && ((presets[curid].buttonIds[stage].Contains("random") || presets[curid].buttonIds[stage].Contains("r")) && !buttons[i].notInRandom))
                        nonbutts.Add(i);
                  }
                  ...
                  foreach (string b in butts)
                  {
                     ...
                     if (bt.useInput)
                     {
                        ...
                        {
                           if (bt.pressed) has++;
                           else if (Input.GetButtonDown(bt.name) || pressed.Contains(bt.name))
                           {
                              bt.pressed = true;
                     ...
                     else
                     {
                        ...
                        {
                           if (bt.pressed) has++;
                           else if (Input.GetKeyDown(bt.key) || pressed.Contains(bt.name))
                           {
                              bt.pressed = true;
                              if (presets[curid].useColor)
                                 bt.obj.GetComponent<Image>().color = presets[curid].ColorPressed;
                  ...
                  if (presets[curid].missing != SLM_QTEEvents_Preset.tpemiss.none)
                  {
                     foreach (int b in nonbutts)
                     {
                        ...
                        if (bt.useInput)
                        {
                           if (Input.GetButtonDown(bt.name))
                              miss = true;
                        }
                        else
            ...
            else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.fast)
            {
               ...
               else
               {
                  ...
                  {
                     if (Input.GetButtonDown(buttons[presets[curid].buttonPossitive].name))
                     {
                        FastPress(true);
                     }
                  }
                  ...
                  {
                     if (Input.GetButtonDown(buttons[presets[curid].buttonNegative].name))
                     {
                        FastPress(false);
                     }
                  }
            ...
            else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.concentrate)
            {
               if (rndtimer > 0)
               {
                  ...
                  if (buttons[presets[curid].buttonPossitive].useInput)
                  {
                     ...
                     else if (Input.GetButtonUp(buttons[presets[curid].buttonPossitive].name))
							{
								concmoveind = 0;
							}
						}
                  else
                  {
                     ...
                     else if (Input.GetKeyUp(buttons[presets[curid].buttonPossitive].key))
							{
								concmoveind = 0;
							}
						}
                  ...
                  if (buttons[presets[curid].buttonNegative].useInput)
                  {
                     ...
                     else if (Input.GetButtonUp(buttons[presets[curid].buttonNegative].name))
							{
								concmoveind = 0;
							}
						}
                  else
                  {
                     if (Input.GetKeyDown(buttons[presets[curid].buttonNegative].key))
                     {
                        concmoveind = -1;
							...
							else if (Input.GetKeyUp(buttons[presets[curid].buttonNegative].key))
							{
								concmoveind = 0;
							}
						}

                  concSlider.value += (concSpeed + (concmoveind * presets[curid].pressSpeed)) * Time.deltaTime;
            ...
            else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.reaction)
            {
               if (concSlider.value < 1)
               {
                  ...
                  {
                     if (Input.GetButtonDown(buttons[presets[curid].buttonReaction].name))
                     {
                        ReactionPress(false);
                     }
                  }
   ...
   void ReactionPress(bool skipped)
   {
      bool fail=false;
   ...
   public void MobilePress(string button)
   {
      if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.reaction)
      {
         if (buttons[presets[curid].buttonReaction].name == button)
            ReactionPress(false);
      }
      else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.concentrate)
      {
         if (buttons[presets[curid].buttonPossitive].name == button)
            concmoveind = 1;
         else if (buttons[presets[curid].buttonNegative].name == button)
            concmoveind = -1;
      }
      else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.fast)
      {
	...
	public void MobilePressDone(string button)
	{
		if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.concentrate)
      {
			if (buttons[presets[curid].buttonPossitive].name == button || buttons[presets[curid].buttonNegative].name == button)
				concmoveind = 0;
		}
		else if (presets[curid].QteType == SLM_QTEEvents_Preset.qtetpe.basic)
		{
         if (presets[curid].mode[stage] != SLM_QTEEvents_Preset.tpe.savePress)
         {
            if (pressed.Contains(button))
